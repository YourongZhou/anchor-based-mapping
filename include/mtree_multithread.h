...
#include <cereal/types/polymorphic.hpp> // 关键：用于处理继承 (Node/Entry)
#include <cereal/types/map.hpp>         // 关键：用于处理 Node::children
#include <cereal/types/string.hpp>       // 用于 Substring
#include <cereal/types/vector.hpp>       // 用于 Substring
#include <cereal/types/memory.hpp>       // 用于智能指针（cereal 内部使用）
...
public:
    ...
    void serialize(Archive& ar) {
        // 序列化所有 M-Tree 的配置参数
        ar(minNodeCapacity, maxNodeCapacity, leafRadiusThreshold,
           minCompactnessThreshold, preSplitRadiusRatio);
...
public:
	...
	public:
		...
		// 默认构造函数，供 cereal 调用
        IndexItem() : radius(0), distanceToParent(-1) {}

		friend class cereal::access;
        template<class Archive>
        void serialize(Archive& ar) {
            ar(data, radius, distanceToParent);
        }
...
public:
	...
	public:
        ...
        void serialize(Archive& ar) {
            // 序列化基类
            ar(cereal::base_class<IndexItem>(this));
		...
		typedef std::map<Data, std::shared_ptr<IndexItem>> ChildrenMap;
	...
	class NonRootNodeTrait : public virtual Node {
		virtual size_t getMinCapacity(const mtree* mtree) const override {
			return mtree->minNodeCapacity;
		}

		virtual void _checkMinCapacity(const mtree* mtree) const override {
